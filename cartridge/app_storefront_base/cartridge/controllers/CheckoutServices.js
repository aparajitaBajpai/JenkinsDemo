...
server.get('Get', server.middleware.https, function (req, res, next) {
    dw.system.Logger.error("under checkout service get");
...
server.post(
    ...
    function (req, res, next) {
        dw.system.Logger.error("under checkout service submitcustomer");
...
server.post(
    ...
    function (req, res, next) {
        ...
        dw.system.Logger.error("submit payment working");
        ...
        this.on('route:BeforeComplete', function (req, res) { // eslint-disable-line no-shadow
            ...
            var stripeHelper = require('*/cartridge/controllers/StripeHelper');
            if (HookMgr.hasHook('app.payment.processor.' + processor.ID.toLowerCase())) {
                dw.system.Logger.error("under if");
                ...
                //mycode
                // result = stripeHelper.handle(currentBasket.getTotalGrossPrice());
                // dw.system.Logger.error("result------> "+JSON.stringify(result));
            } else {
                dw.system.Logger.error("under else");
...
server.post('PlaceOrder', server.middleware.https, function (req, res, next) {
    ...
    var currentBasket = BasketMgr.getCurrentBasket();
    dw.system.Logger.error("under checkout service place order");
    ...
    dw.system.Logger.error("under place order line 1");

    // Re-validates existing payment instruments
    //var validPayment = COHelpers.validatePayment(req, currentBasket);
    // my code
    var stripeHelper = require('*/cartridge/controllers/StripeHelper');
    var validPayment = stripeHelper.finalHandle();
    dw.system.Logger.error("-->"+validPayment.error);
    ...
    dw.system.Logger.error("under place order line 2");
    ...
    // //mycode
    // //var handlePaymentResult = {error: false};
